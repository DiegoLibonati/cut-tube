...
# ─── CONFIG ──────────────────────────────────────────────────────────────────
PACKAGES=("cut-tube-api" "cut-tube-app")
NAMES=("API" "App")
# Runner per package: "pytest" or "npm"
RUNNERS=("pytest" "npm")
NPM_SCRIPT="test"
NPM_CLIENT="npm"
# ─────────────────────────────────────────────────────────────────────────────
...
for i in "${!PACKAGES[@]}"; do
  ...
  (
    cd "$ROOT/$dir" || exit 1
    if [ "${RUNNERS[$i]}" = "pytest" ]; then
      source venv/Scripts/activate && pytest > "$outfile" 2>&1
    else
      $NPM_CLIENT run $NPM_SCRIPT > "$outfile" 2>&1
    fi
